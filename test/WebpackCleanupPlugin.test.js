...
describe('WebpackCleanupPlugin', () => {
  it('should set options from constructor', () => {
    const plugin = new WebpackCleanupPlugin({
      quiet: true,
      preview: true,
      exclude: ['a.txt'],
    });
  ...
  describe('when deleting files', () => {
    ...
    it('should delete the extraneous files from the output path', done => {
      ...
      compiler.run(() => {
        ...
        expect(unlinkSync).to.not.have.been.calledWith(
          njoin(outputPath, 'bundle.js'),
        );
        expect(unlinkSync).to.have.been.calledWith(
          njoin(outputPath, 'foo.json'),
        );
    ...
    it('should not delete when previewing files', done => {
      const compiler = webpack({
        ...webpackConfig,
        plugins: [new WebpackCleanupPlugin({ preview: true })],
      });
      compiler.run(() => {
    ...
    it('should only delete files created by webpack', done => {
       ...
       */
      webpackCleanupPlugin.previousAssets = [
        'bundle.js',
        'a.txt',
        'folder/q.txt',
      ];
      ...
      compiler.run(() => {
        expect(webpackCleanupPlugin.previousAssets).to.deep.equal([
          'bundle.js',
        ]);
        expect(unlinkSync).to.have.been.calledWith(njoin(outputPath, 'a.txt'));
        expect(unlinkSync).to.have.been.calledWith(
          njoin(outputPath, 'folder/q.txt'),
        );

        expect(unlinkSync).to.not.have.been.calledWith(
          njoin(outputPath, 'folder/p.txt'),
        );
        expect(unlinkSync).to.not.have.been.calledWith(
          njoin(outputPath, 'b.txt'),
        );
        expect(unlinkSync).to.not.have.been.calledWith(
          njoin(outputPath, 'bundle.js'),
        );
